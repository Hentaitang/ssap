...
(function () {
  ...
  var inlineImages = function inlineImages(el, type) {
    if (type) {
      ...
      return Promise.all(Array.from(el.querySelectorAll('image')).map(function (image) {
        var ins = window.location.href.split('/')
        var inside = ins[0] + '/' + ins[1] + '/' + ins[2] + '/'

        var href = inside + 'img/abstract-user-1.png'
  ...
  out$.prepareSvg = function (el, options, done, type) {
    requireDomNode(el);
    ...
    return inlineImages(el, type).then(function () {
  ...
  out$.svgAsDataUri = function (el, options, done, type) {
    requireDomNode(el);
    var result = out$.prepareSvg(el, options, '', type).then(function (_ref5) {
      var src = _ref5.src,
        width = _ref5.width,
        height = _ref5.height;
  ...
  out$.svgAsPngUri = function (el, options, done, type = true) {
    requireDomNode(el);
    ...
    if (canvg) return out$.prepareSvg(el, options, '', type).then(convertToPng);
    else return out$.svgAsDataUri(el, options, '', type).then(function (uri) {
      return new Promise(function (resolve, reject) {
        var image = new Image();
